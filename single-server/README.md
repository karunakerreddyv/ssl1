...
> **For forks:** Set `REGISTRY=<your-dockerhub-user>` (or `REGISTRY=ghcr.io/<your-org>/pravaha` for GHCR) and adjust `IMAGE_PREFIX` accordingly in `.env`.

## Container Registry Configuration

### Default Registry: Docker Hub (`karunakervgrc1`)
Images are pulled from Docker Hub (private repos under the `karunakervgrc1` namespace) by default. Pulling private images requires authentication — see "Registry Authentication" below.

**Images:**
- `karunakervgrc1/pravaha-frontend:latest`
- `karunakervgrc1/pravaha-backend:latest`
- `karunakervgrc1/pravaha-superset:latest`
- `karunakervgrc1/pravaha-ml-service:latest`

GHCR is also supported as an alternative (set `REGISTRY=ghcr.io/<your-org>/pravaha` and `IMAGE_PREFIX=` in `.env`).

### Registry Authentication
Set environment variables before running install.sh so it can pull private images:

```bash
# Docker Hub (default — uses an access token, not your master password)
export DOCKER_USERNAME=karunakervgrc1
export DOCKER_PASSWORD=dckr_pat_<read-only-access-token>

# OR for GHCR
export GHCR_USERNAME=<github-username>
export GHCR_TOKEN=ghp_<personal-access-token-with-read:packages>

sudo -E ./scripts/install.sh --domain <your-domain-or-IP> --ssl selfsigned
```

`-E` preserves env vars when running with sudo. install.sh's `authenticate_registry()` detects the registry type and logs in automatically.
...
The docker-compose.yml expects images from Docker Hub under `karunakervgrc1/pravaha-*`. Tag your locally built images to match:

```bash
# Tag each locally built image with the expected registry name
# Format: docker tag <local-name>:latest karunakervgrc1/<service>:latest

docker tag single-server-frontend:latest karunakervgrc1/pravaha-frontend:latest
docker tag single-server-backend:latest karunakervgrc1/pravaha-backend:latest
docker tag single-server-ml-service:latest karunakervgrc1/pravaha-ml-service:latest
docker tag single-server-superset:latest karunakervgrc1/pravaha-superset:latest

# Verify tags were applied
docker images | grep karunakervgrc1
```
...
docker save karunakervgrc1/pravaha-frontend:latest | gzip > pravaha-images/frontend.tar.gz
docker save karunakervgrc1/pravaha-backend:latest | gzip > pravaha-images/backend.tar.gz
docker save karunakervgrc1/pravaha-ml-service:latest | gzip > pravaha-images/ml-service.tar.gz
docker save karunakervgrc1/pravaha-superset:latest | gzip > pravaha-images/superset.tar.gz
...
for svc in frontend backend ml-service superset; do \
  docker tag single-server-$svc:latest karunakervgrc1/$svc:latest && \
  docker save karunakervgrc1/$svc:latest | gzip > $svc.tar.gz; \
done
...
```bash
docker images | grep karunakervgrc1
# Expected: All 4 images from karunakervgrc1/*

# Verify architecture
docker inspect karunakervgrc1/pravaha-frontend:latest --format='{{.Architecture}}'
...
docker pull karunakervgrc1/pravaha-frontend:latest
docker pull karunakervgrc1/pravaha-backend:latest
