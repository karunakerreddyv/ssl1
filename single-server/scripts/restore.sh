...
restore_volumes() {
    ...
    if [[ -f "$BACKUP_DIR/solution_packs.tar.gz" ]]; then
        log_info "  Restoring solution packs..."
        compose_cmd exec -T backend tar xzf - -C / < "$BACKUP_DIR/solution_packs.tar.gz" || log_warning "Could not restore solution packs"
    fi
...
case $RESTORE_TYPE in
    ...
    *)
        ...
        if [[ -f "$BACKUP_DIR/uploads.tar.gz" ]] || [[ -f "$BACKUP_DIR/ml_models.tar.gz" ]] || [[ -f "$BACKUP_DIR/superset_home.tar.gz" ]] || [[ -f "$BACKUP_DIR/ml_storage.tar.gz" ]] || [[ -f "$BACKUP_DIR/training_data.tar.gz" ]] || [[ -f "$BACKUP_DIR/plugins.tar.gz" ]] || [[ -f "$BACKUP_DIR/solution_packs.tar.gz" ]] || [[ -f "$BACKUP_DIR/celery_beat_schedule.tar.gz" ]] || [[ -f "$BACKUP_DIR/workflow_configs.tar.gz" ]] || [[ -f "$BACKUP_DIR/jupyter_notebooks.tar.gz" ]] || [[ -f "$BACKUP_DIR/jupyter_data.tar.gz" ]]; then
            restore_volumes
        fi
        ;;
esac
