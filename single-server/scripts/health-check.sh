...
# Ollama LLM (optional — only check if container exists)
if docker ps -a --format '{{.Names}}' | grep -q "pravaha-ollama"; then
    check_container "ollama" false
fi
...
if [[ "$QUICK_MODE" != "true" ]]; then
    ...
    # Ollama LLM (optional — check endpoint and model count)
    if docker ps --format '{{.Names}}' | grep -q "pravaha-ollama"; then
        check_internal_service "Ollama" "ollama" "http://localhost:11434/api/tags" false
        ollama_model_count=$(docker exec pravaha-ollama curl -sf http://localhost:11434/api/tags 2>/dev/null | grep -o '"name"' | wc -l || echo "0")
        log_check "Ollama models" "pass" "${ollama_model_count} model(s) installed" "false"
    fi
