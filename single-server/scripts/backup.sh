...
backup_volumes() {
    ...
    compose_cmd exec -T backend tar czf - /app/solution-packs 2>/dev/null \
        > "$BACKUP_DIR/$BACKUP_NAME/solution_packs.tar.gz" || log_warning "No solution packs to backup"
...
cat > "$BACKUP_DIR/$BACKUP_NAME/manifest.json" << EOF
{
    ...
    "contents": {
        ...
        "volumes": $(if [[ "$BACKUP_TYPE" == "full" ]]; then echo '["uploads", "ml_models", "superset_home", "ml_storage", "training_data", "plugins", "solution_packs", "celery_beat_schedule", "workflow_configs", "jupyter_notebooks", "jupyter_data"]'; else echo '[]'; fi),
