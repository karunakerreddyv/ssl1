...
start_services() {
    ...
    # Check if Ollama LLM is enabled
    local enable_ollama="${OLLAMA_ENABLED:-false}"
    if [[ "$enable_ollama" == "true" ]]; then
        compose_flags+=("--profile" "llm")
        log_info "Local LLM (Ollama) enabled — will pull model after services start"
    fi

    # Start services with appropriate flags
    log_info "Starting services (postgres=$pg_mode, logging=$enable_logging, ollama=$enable_ollama)..."
    ...
    for service in "${services[@]}"; do
        ...
        if ! wait_for_healthy "$service" "$max_attempts" 5; then
            ...
            case "$service" in
                *celery*|*ollama*|*loki*|*promtail*|*grafana-logs*)
    ...
    # Pull default Ollama model if LLM is enabled
    if [[ "$enable_ollama" == "true" ]]; then
        local ollama_model="${OLLAMA_DEFAULT_MODEL:-llama3.1}"
        log_info "Pulling Ollama model: $ollama_model (this may take several minutes for first download)..."
        if docker exec pravaha-ollama ollama pull "$ollama_model" 2>&1; then
            log_success "Ollama model '$ollama_model' pulled successfully"
            # Set AI_MOCK_MODE=false since we have a real LLM now
            if grep -q "AI_MOCK_MODE=true" "$deploy_dir/.env" 2>/dev/null; then
                sed -i "s|AI_MOCK_MODE=true|AI_MOCK_MODE=false|g" "$deploy_dir/.env"
                log_info "Set AI_MOCK_MODE=false (real LLM available)"
            fi
        else
            log_warning "Failed to pull Ollama model '$ollama_model' — you can pull it later with:"
            log_warning "  docker exec pravaha-ollama ollama pull $ollama_model"
        fi
    fi
}
...
main() {
    ...
    if [[ "$dry_run" == "true" ]]; then
        ...
        if [[ "$skip_pull" == "true" ]]; then
            ...
            echo "      - Pull images from ${REGISTRY:-karunakervgrc1}/*:${IMAGE_TAG:-latest}"
        fi
        echo ""
    ...
    # =========================================================================
    # STEP 12b: Demo-user cleanup (FRESH INSTALL ONLY)
    # =========================================================================
