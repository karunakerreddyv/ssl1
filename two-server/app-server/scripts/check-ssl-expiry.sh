...
#   Provides alerts for expiring certificates and optionally triggers renewal.
#
# Usage:
...
check_certificates() {
    ...
    if ! check_cert_exists "$cert_file"; then
        ...
        if [[ "$JSON_OUTPUT" == "true" ]]; then
            echo '{"status":"error","message":"Certificate file not found","deployment_type":"two-server-app"}'
        fi
        return 2
    fi
...
log_to_audit "CHECK" "SSL certificate check started for domain: $DOMAIN (two-server-app)"
check_certificates
exit_code=$?
