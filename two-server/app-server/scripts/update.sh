#!/bin/bash
# =============================================================================
# Pravaha Platform - Update Script (Two-Server App Server)
...
#   - External PostgreSQL on separate database server (NO bundled postgres)
#   - Database connectivity validation before update
#   - Ordered service startup (redis -> rest, no postgres locally)
...
init_update_log() {
    ...
    cat > "$UPDATE_LOG" << EOF
# =============================================================================
# Pravaha Platform - Update Log (Two-Server App Server)
...
# =============================================================================
# compose_cmd()
# Wrapper around docker compose. Two-server app: always external database,
# no profile needed.
# =============================================================================
compose_cmd() {
...
get_service_list() {
    ...
    # Infrastructure services (no postgres - runs on Database Server)
...
# 1. Validate external Database Server connectivity
...
start_services_ordered() {
    ...
    # Validate external Database Server connectivity
    log_info "  Validating external Database Server connectivity..."
    local db_host="${POSTGRES_HOST:-}"
    local db_port="${POSTGRES_PORT:-5432}"
    if [[ -n "$db_host" ]]; then
        if timeout 10 bash -c "echo > /dev/tcp/$db_host/$db_port" 2>/dev/null; then
            log_success "  Database Server ($db_host:$db_port) is reachable"
        else
            log_warning "  Database Server ($db_host:$db_port) is not reachable - services may fail"
        fi
    else
        log_warning "  POSTGRES_HOST not set, cannot validate database connectivity"
    fi

    # Step 1: Start Redis (no local postgres - runs on Database Server)
    ...
    # Step 2: Start all remaining services
...
echo "=============================================="
echo "Pravaha Platform - Update (Two-Server App Server)"
echo "=============================================="
...
if [[ "$DRY_RUN" == "true" ]]; then
    ...
    echo "  10. Pull new Docker images (with retry)"
    echo "  11. Stop services gracefully (compose_cmd down)"
    echo "  12. Start services in order:"
    echo "      a. Validate external Database Server connectivity"
    ...
    echo "      a. Wait for external Database Server + backend"
    ...
    echo ""
    echo "Note: PostgreSQL is on the external database server."
    echo "      Database schema changes should be coordinated with the DB admin."
    exit 0
fi
...
cat > "$CHECKPOINT_PATH/manifest.json" << EOF
{
    ...
    "domain": "${DOMAIN}",
    "external_db_host": "${POSTGRES_HOST:-unknown}",
...
if ! start_services_ordered; then
    ...
    # Wait for backend to be ready (needed for migrations)
    # Note: PostgreSQL is on the external database server
...
if [[ "$ALL_HEALTHY" == "true" ]]; then
    echo "=============================================="
    log_success "App Server update to $NEW_VERSION completed successfully!"
    echo "=============================================="
    echo ""
    compose_cmd ps
    echo ""
    ...
    echo "=============================================="
    log_error "App Server update to $NEW_VERSION FAILED!"
    echo "=============================================="
    echo ""
