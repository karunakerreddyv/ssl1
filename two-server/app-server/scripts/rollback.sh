...
#
# Purpose:
#   Rollback to previous state after failed update on the app server.
#   Restores configuration, images, and services to a checkpoint state.
#
# Usage:
...
#
# Architecture Notes:
#   - Rollback is for app server Docker services only
#   - Database rollback must be coordinated with database server
#   - Redis data is local and will be restored if volume backup exists
#
# =============================================================================
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo ""
            echo "Note: This rollback is for app server only."
            echo "      Database changes must be rolled back on the database server."
            exit 0
            ;;
        *)
...
list_checkpoints() {
    ...
    if [[ ! -d "$CHECKPOINT_DIR" ]] || [[ -z "$(ls -A "$CHECKPOINT_DIR" 2>/dev/null | grep -v latest)" ]]; then
...
verify_checkpoint() {
    ...
    # Check external database host
    local checkpoint_db_host=$(jq -r '.external_db_host // "unknown"' "$checkpoint_path/manifest.json" 2>/dev/null)
    local current_db_host="${POSTGRES_HOST:-unknown}"

    if [[ "$checkpoint_db_host" == "$current_db_host" ]]; then
        log_success "External DB Host: $current_db_host (matches checkpoint)"
    else
        log_warning "External DB Host: $current_db_host (checkpoint: $checkpoint_db_host)"
    fi
...
check_health() {
    ...
    # App server services (no postgres - external on database server)
...
rollback() {
    ...
    echo "=============================================="
    echo "Pravaha Platform - Rollback (App Server)"
    echo "=============================================="
    echo "Deployment: Two-Server (App Server)"
    echo "Target:     $target"
    echo ""
    ...
    if [[ "$AUTO_ROLLBACK" != "true" ]]; then
        log_warning "This will restore the app server to checkpoint: $target"
        log_warning "Database changes must be rolled back separately on the database server."
    ...
    # Re-source environment after .env restoration
    if [[ -f "$DEPLOY_DIR/.env" ]]; then
        set -a
        source "$DEPLOY_DIR/.env"
        set +a
    ...
    # Step 6: Start services (ordered startup)
    ...
    # Step 8: Verify external database connectivity
    ...
    if [[ -x "$SCRIPT_DIR/validate-external-db.sh" ]]; then
        if "$SCRIPT_DIR/validate-external-db.sh" --quiet; then
            ...
            log_warning "Could not verify external database connection"
        fi
    fi
    ...
    if [[ -f "$checkpoint_path/manifest.json" ]]; then
        ...
        log_warning "Note: This rollback restored app server configuration and images only."
        log_warning "If database rollback is also needed, coordinate with database server admin."
        log_warning "Checkpoint was created at: $checkpoint_time"
    fi
}
...
if [[ "$LIST_CHECKPOINTS" == "true" ]]; then
    ...
    verify_checkpoint
elif [[ "$AUTO_ROLLBACK" == "true" ]] && [[ -z "$ROLLBACK_TARGET" ]]; then
    auto_rollback
else
    rollback
fi
